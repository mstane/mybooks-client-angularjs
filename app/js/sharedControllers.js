...
sharedControllers.controller('navigation', function($rootScope, $scope, $http, $location, $route, NotificationService) {
	...
	$scope.forgottenPasswordSend = function() {
		$http.get('http://localhost:8080/forgotten_password_send', { params : { email : $scope.email } }).success(function(data, status, headers, config) {
	...
	$scope.register = function() {
		$http.post('http://localhost:8080/register_reader', $scope.reader).success(function(data, status, headers, config) {
			$location.path("/");
